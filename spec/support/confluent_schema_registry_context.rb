...
shared_examples_for 'a confluent schema registry client' do |auth_mechanism|
  ...
  describe '#global_config' do
    ...
    it 'returns the global configuration' do
      expect(registry.global_config).to eq(maybe_include_auth_header_in_expected_config(JSON.parse(expected),
                                                                                        auth_mechanism))
    end
  end
  ...
  describe '#update_global_config' do
    ...
    it 'updates the global configuration and returns it' do
      expect(registry.update_global_config(config)).to eq(maybe_include_auth_header_in_expected_config(
                                                            JSON.parse(expected), auth_mechanism
                                                          ))
      expect(registry.global_config).to eq(maybe_include_auth_header_in_expected_config(JSON.parse(expected),
                                                                                        auth_mechanism))
    end
  ...
  describe '#subject_config' do
    ...
    context 'when the subject config is not set' do
      it 'returns the global configuration' do
        expect(registry.subject_config(subject_name)).to eq(maybe_include_auth_header_in_expected_config(
                                                              JSON.parse(expected), auth_mechanism
                                                            ))
      end
    end
